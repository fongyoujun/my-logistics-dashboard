...
def show_logistics_dashboard():
    ...
    with st.sidebar:
        ...
        # 修正：这里只保留一个循环，避免 DuplicateKey 报错
        active_list = []
        for name in charts_config.keys():
            if st.checkbox(name, value=True, key=f"active_v2_{name}"):
                active_list.append(name)

        # --- 核心：增加刷新功能按钮 ---
        st.markdown("---")
        if st.button("🔄 Sync GitHub Data", key="refresh_btn_unique_99"):
...
def show_openpo_dashboard():
    ...
    with st.sidebar:
        st.markdown("---")
        if st.button("🔄 Refresh OpenPO Data", key="refresh_btn_openpo"):
            st.cache_data.clear()
            st.rerun()
    try:
        df = pd.read_excel("openpo.xlsx", header=[0, 1])
    except FileNotFoundError:
        st.error("未找到 openpo.xlsx 文件。")
        return
    ...
    for label, color, symbol, cost, cbm, po, cp_cbm, cp_po, is_vis in groups:
        ...
        fig.add_trace(
            go.Scatter(x=df['Timeline'], y=po, name=f"{label} PO", line=dict(color=color, width=2, dash='dash'),
                       visible=vis))
        fig.add_trace(
            go.Scatter(x=df['Timeline'], y=cp_cbm, name=f"{label} AVG COST/CBM", yaxis='y2', visible="legendonly",
                       mode='lines+markers', marker=dict(symbol=symbol, size=8), line=dict(color=color, width=2.5)))
        fig.add_trace(
            go.Scatter(x=df['Timeline'], y=cp_po, name=f"{label} AVG COST/PO", yaxis='y2', visible="legendonly",
